...
<body style="background-color: white;">
	...
	<div id="centerer">
        <?php
         if ($_GET['pass'] != "terry"){
             echo(' <div><form method="GET" action="">Admin Password: <input value="<?php echo($rnum);?>" name="id" hidden type="textbox"/><input name="pass" type="textbox"/> <input type="submit" value="Submit"></form></div>');
         }
        ?>
       
		<div class="Rounded stripes">
			<a href="../../index.php" id="backLink"><span id="backicon"></span><h2 id="backText">Back</h2></a>
    ...
    stmt_bind_assoc($stmt, $row);
         while ($stmt->fetch()) {
			...
			<table id="serverTable">
				...
				<tr><td><button style="font-size:12px;" id="b1" onclick="goToServer('.$row['ID'].')" class="button button2">View Details</button></td><td><button style="font-size:12px;" id="b1" onclick="goToEditServer('.$row['ID'].')" class="button button2">Edit</button></td></tr>
			</table>
		</div>');
             }else{
                         echo('<div id="Item" class="Rounded serverItem" style="background-color:white;height:100px;">
...
<footer>
	<script>
        function goToServer(num){
            window.location = "../server/?id=" + num;
        }
        function goToEditServer(num){
            window.location = "../admin/editServer.php?id=" + num;
        }
		function search(){
